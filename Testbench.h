...
public:
	...
	sc_out < bool >  o_rst;
#ifndef NATIVE_SYSTEMC
	cynw_p2p< sc_dt::sc_uint<8> >::base_out o_r[9];
  cynw_p2p< sc_dt::sc_uint<8> >::base_out o_g[9];
  cynw_p2p< sc_dt::sc_uint<8> >::base_out o_b[9];
	cynw_p2p< sc_dt::sc_uint<32> >::base_in i_result;
  // The base_in and base_out metaports are simpler versions of the in and out metaports intended for
  //testbenches, and for unpipelined synthesizable modules. The base ports do not require binding to clock
  //and reset signals because they do not contain SC_METHODs.

#else
	sc_fifo_out< sc_dt::sc_uint<8> > o_r[9];
  sc_fifo_out< sc_dt::sc_uint<8> > o_g[9];
  sc_fifo_out< sc_dt::sc_uint<8> > o_b[9];
